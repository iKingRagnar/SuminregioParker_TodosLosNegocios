 ...
 */
function sqlWhereCotizacionFast(alias = 'd', src = 'VE') {
  const a = alias;
  const reqAplicado = (process.env.MICROSIP_COTIZACION_REQUIERE_APLICADO || '').match(/^(1|true|yes)$/i);
  const aplicado = reqAplicado ? ` AND COALESCE(${a}.APLICADO, 'N') = 'S'` : '';
...
// Endpoints de metas/objetivos — extraídos a src/routes/metas-routes.js
// (refactor incremental). Se inyectan los helpers del server que necesitan.
require('./src/routes/metas-routes').install({
  app, get, query, normalizeDbQueryId, isAllDbs,
  mapPoolLimit, DATABASE_REGISTRY, getReqDbOpts,
...
get('/api/inv/existencias-todas', async (req) => {
  ...
  try {
    ...
    if (probe) {
      claveJoin = 'LEFT JOIN ( SELECT ARTICULO_ID, MAX(CLAVE_ARTICULO) AS CLAVE FROM CLAVES_ARTICULOS GROUP BY ARTICULO_ID ) ka ON ka.ARTICULO_ID = a.ARTICULO_ID';
      claveSel = OCT("COALESCE(ka.CLAVE, '')", 80) + ' AS CLAVE';
    }
  } catch (_) { /* sin tabla de claves: se entrega vacia */ }
  const rows = await query(`
    SELECT FIRST ${limit} a.ARTICULO_ID, ${claveSel}, ${OCT('a.NOMBRE', 255)} AS DESCRIPCION,
      ${OCT("COALESCE(a.UNIDAD_VENTA, 'PZA')", 40)} AS UNIDAD, ${unidadCompra} AS UNIDAD_COMPRA,
  ...
  return (rows || []).map((r) => {
    r.CLAVE = dec(r.CLAVE);
    r.DESCRIPCION = dec(r.DESCRIPCION);
    r.UNIDAD = dec(r.UNIDAD);
    r.UNIDAD_COMPRA = dec(r.UNIDAD_COMPRA);
    return r;
  });
});
...
// ══════════════════════════════════════════════════════════════════════════════
// ===== ENDPOINTS AUTO-AGREGADOS: OC / CM / venta-buscar (cadena de documentos) =====
// ════════════════════════════════════════════════════════════════════════════
...
//  El cliente SNTE (RFC SNT391220717) se filtra por defecto.
