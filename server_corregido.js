...
// ── Multi-FDB: lista desde FB_DATABASES_JSON; si no hay, solo la base por defecto ──
function parseDatabaseRegistry() {
  ...
  const password = process.env.FB_PASSWORD || 'masterkey';
  const raw = process.env.FB_DATABASES_JSON;
  if (raw && raw.trim()) {
    try {
      const arr = JSON.parse(raw);
      if (!Array.isArray(arr) || !arr.length) throw new Error('FB_DATABASES_JSON debe ser un array no vacío');
      return arr.map((x, i) => {
        if (!x || !x.database) throw new Error(`Entrada ${i}: falta "database"`);
        return {
          id: String(x.id || `empresa_${i + 1}`),
          label: String(x.label || x.id || `Empresa ${i + 1}`),
          options: {
            host: x.host || host,
            port: parseInt(x.port, 10) || port,
            database: x.database,
            user: x.user || user,
            password: x.password != null ? x.password : password,
  ...
  return [{
    id: 'default',
    label: process.env.EMPRESA_NOMBRE || 'Principal',
    options: { ...DB_OPTIONS },
  }];
}
...
get('/api/universe/scorecard', async (req) => {
  ...
  const qms = Math.min(Math.max(parseInt(req.query.queryMs, 10) || 10000, 3000), 60000);
...
get('/api/resultados/pnl', async (req) => {
  const dbId = req.query.db ? String(req.query.db) : '';
  if (dbId) {
    const hit = DATABASE_REGISTRY.find(d => d.id === dbId);
    if (!hit) throw new Error('Parámetro db desconocido: ' + dbId + '. Ver GET /api/universe/databases');
...
function aiResolveDbOpts(req) {
  ...
  const dbId = (body.db != null && String(body.db).trim() !== '' ? String(body.db) : '') ||
    (req.query && req.query.db ? String(req.query.db) : '');
  if (!dbId) return { opts: null, id: '', label: '' };
  const hit = DATABASE_REGISTRY.find(d => d.id === dbId);
  if (!hit) {
    return {
