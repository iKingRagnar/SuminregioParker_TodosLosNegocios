...
// ── Cumplimiento: compara valor REAL vs META (delta + % alcance) ─────────────
//   Dirección desde el esquema: '≥' = más es mejor; '≤' = menos es mejor.
function dirOf(key) {
  const s = SCHEMA_BY_KEY[key];
  return (s && s.dir === '≤') ? 'lower' : 'higher';
}

/**
 * @param {string} key   meta (para conocer la dirección)
 * @param {number} real  valor actual medido (mismas unidades que la meta)
 * @param {number} meta  valor objetivo
 * @returns {{ real, meta, delta, pct, alcanzada, dir }}
 *   delta = real − meta
 *   pct   = % de cumplimiento/alcance (direccional): 100% = justo en la meta,
 *           >100% = mejor que la meta, <100% = por debajo.
 *   alcanzada = ¿se cumple el objetivo?
 */
function cumplimiento(key, real, meta) {
  const dir = dirOf(key);
  const m = Number(meta);
  if (real == null || !isFinite(Number(real)) || !isFinite(m)) {
    return { real: (real == null ? null : Number(real)), meta: m, delta: null, pct: null, alcanzada: null, dir };
  }
  const r = Number(real);
  const delta = Math.round((r - m) * 1e6) / 1e6;
  let pct;
  if (dir === 'higher') {
    // Con meta ≤ 0 (p.ej. crecimiento YoY con objetivo 0/negativo) la razón r/m no es válida;
    // se reporta 100% si se alcanza y 0% si no — nunca null cuando el real es finito (antes
    // un negocio en caída quedaba como "sin dato" y no levantaba brecha).
    pct = m > 0 ? (r / m) * 100 : (r >= m ? 100 : 0);
  } else { // lower-is-better: cumplir = quedar por debajo del techo
    pct = r > 0 ? (m / r) * 100 : (r <= m ? 100 : 0);
  }
  const alcanzada = dir === 'higher' ? r >= m : r <= m;
  return { real: r, meta: m, delta, pct: pct == null ? null : Math.round(pct * 10) / 10, alcanzada, dir };
}

module.exports = {
  SCHEMA, DERIVED, SCHEMA_BY_KEY,
  defaults, loadOverrides, saveOverrides, validateMerge, resetOverrides,
  buildPayload, dirOf, cumplimiento, OVERRIDES_PATH,
};
