...
async function main() {
  try {
    ...
    const tests = [
      ...
      { label: 'resultados/pnl',           url: '/api/resultados/pnl?db=parker&anio=2026&mes=3&meses=6' },
      { label: 'resultados/balance-general', url: '/api/resultados/balance-general?db=parker' },
      { label: 'resultados/estado-sr',     url: '/api/resultados/estado-sr?db=parker&anio=2026&mes=3' },
    ];
    ...
    for (const t of tests) {
      try {
        ...
        if (t.label === 'resultados/pnl' && r.json && r.json.totales) {
          const tot = r.json.totales;
          console.log(`      VENTAS_NETAS=${fmtMoney(tot.VENTAS_NETAS)}  UTILIDAD_BRUTA=${fmtMoney(tot.UTILIDAD_BRUTA)}  meses=${(r.json.meses||[]).length}`);
        }
        if (t.label === 'resultados/balance-general' && r.json && r.json.totales) {
          const tot = r.json.totales;
          console.log(`      ACTIVO=${fmtMoney(tot.ACTIVO_TOTAL)}  PASIVO=${fmtMoney(tot.PASIVO_TOTAL)}  CAPITAL=${fmtMoney(tot.CAPITAL_TOTAL)}  DIFF=${fmtMoney(tot.DIFERENCIA_BALANCE)}`);
        }
      } catch (e) {
        fail++;
